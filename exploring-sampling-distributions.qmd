...
output$bootDistPlot <- renderPlot({
  ...
  sample_mean <- mean(salt_samples$na_ppm)
  std_err <- sd(boot_dist_data()$mean_ppm)
  dist_mean <- mean(boot_dist_data()$mean_ppm)
  ...
  plot <- boot_dist_data() |> 
    ...
    geom_vline(xintercept=sample_mean, linewidth=1, linetype="dashed", colour="grey40") +
  ...
  y_anchor <- max(ggplot_build(plot)$data[[1]]$count)/2
    
  plot +
    annotate("text", x=ci_lower-0.05, y=y_anchor,label="Lower end of confidence interval", angle=90, size=6) +
    annotate("text", x=ci_upper+0.05, y=y_anchor,label="Upper end of confidence interval", angle=270, size=6) +
    labs(
      x="Resample mean",
      y="Count",
      title = str_glue("The bootstrap distribution with {input$boot_replications} resamples")
    )
})
