...
output$bootDistPlot <- renderPlot({
  ...
  y_anchor2 <- max(ggplot_build(plot)$data[[1]]$count)
    
  plot +
    annotate("text", x=sample_mean+0.05, y=y_anchor2*0.9, size=5, 
             label=str_glue("Original sample mean: {sample_mean}"), colour="firebrick", hjust="left") +
    annotate("text", x=dist_mean-0.05, y=y_anchor2*0.95, size=5, 
